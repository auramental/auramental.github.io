  ...
  app.post("/push", function(req,res) {
    try {
        ...
        if(com.split(req.body.entryName).length == 1) {
            com = '<tr><td class="short-name"><a href="data/' + req.body.entryName +'.html">' + vars["TITLE"] + '</a></td><td>' + (dt.getDate() + "/" + (dt.getMonth()+1) + "/" + dt.getFullYear()) + '</td></tr>' + com;
            fs.writeFileSync("strings/commits.txt",com);           
        }
        ind = fs.readFileSync("template/index.html").toString();
        ind = ind.split("%COMMITS%").join(com.split("</tr>").slice(0,20).join("</tr>") + "<tr><td><a href='archive.html'>...</td><td></td></tr>");
        fs.writeFileSync("index.html",ind);
        ind = fs.readFileSync("template/index.html").toString();
        ind = ind.split("%COMMITS%").join(com);
        fs.writeFileSync("archive.html",ind);        
        res.send({});
    } catch(e) {
...
function make(latex,entryName,magicCode,ch,res) {
    ...
    vars = JSON.parse(vars);
    try {
        fs.unlinkSync('sandbox/' + entryName + '.pdf');
    } catch(e) {
        console.log("nothing");
    }
